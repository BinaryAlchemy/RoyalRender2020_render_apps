...
class JobFileExport(object):
    ...
    def addJob(self, rr_job):
        ...
        writeInfo("JobFileExport.addJob. Count %d " % (len(self._rr_jobs)))
    ...
    def _writeToXMLJob(self):
        for rr_job in self._rr_jobs:
            writeInfo("_writeToXMLJob newJob")
            ...
            if len(rr_job.rendererVersionName) > 0:
                ...
                self._sub_e(jobElement, "rendererVersion", rr_job.rendererVersion)
            ...
            if (rr_job.customGSV != ""):
                self._sub_e(jobElement, "customGSV", rr_job.customGSV )
    ...
    def _writeToXMLEnd(self, use_temp_file=True):
        ...
        writeInfo("_writeToXMLEnd %s" % (self._export_file.name) )
...
def createSubmitJob(nodeRenderSettings, local_export=False):

    writeInfo("createSubmitJob: renderNode %s. " % (nodeRenderSettings.nodeName))
    
    # create job
    newJob = RRJob()

    if not newJob.set_software(nodeRenderSettings.renderService, local_export):
        writeError("software info collection failed, aborting")
        return
    
    if local_export:
        file_dir, file_name = get_local_path(nodeRenderSettings.customFarmSettings)
        newJob.set_local_scene_export(file_dir, file_name)
    else:
        newJob.sceneName = FarmAPI.GetKatanaFileName()
        

    writeDebug("Dependency list: " + str(nodeRenderSettings.dependencies))

    if not newJob.set_output(nodeRenderSettings.outputs):
        writeError("no output path found, aborting")
        return
    if not newJob.set_framerange(nodeRenderSettings.frameRange):
    ...
    newJob.camera = ""
    newJob.layer = nodeRenderSettings.nodeName
    ...
    if local_export:
        ...
        export_scene_frames(newJob, nodeRenderSettings.renderService)
        
    return newJob


def submitJobList(jobList):
    # write xml file
    job_export = JobFileExport()
    
    for job in jobList:
        job_export.addJob(job)
