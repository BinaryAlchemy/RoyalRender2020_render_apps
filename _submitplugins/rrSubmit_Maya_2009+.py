...
class rrMayaLayer:
    ...
    def getLayerSettings_VRay(self,Layer,DatabaseDir,SceneName,MayaVersion,isLayerRendering,CurrentLayer,LayerOverrides,LayerOverridesMaster):
        ...
        if (isVRay20):
            rgbaOut_Subfolder=cmds.getAttr('vraySettings.relements_separateRGBA')
...
class rrPlugin(OpenMayaMPx.MPxCommand):
        ...
        if (len(RenderLayer)>1):
            for Layer in RenderLayer:
                ...
                if (mayaUpd2_layerName):
                    mayaUpd2_layerName=False; #first check completed, we have the reuqired new maya version 
                    ...
                    if (self.layer[self.maxLayer-1].renderer=="redshift"):
                        redVersion=cmds.pluginInfo( 'redshift4maya', query=True, version=True )
                        if (redVersion>="2.0.88"):
                            mayaUpd2_layerName=True;
                    if (self.layer[self.maxLayer-1].renderer=="vray"):
                        renVersion=cmds.pluginInfo( 'vrayformaya', query=True, version=True )
                        if (renVersion>="3.52"):
                            mayaUpd2_layerName=True;
                            
                #Maya 2017 update 2 fix:
                if (mayaUpd2_layerName and self.layer[self.maxLayer-1].name.startswith("rs_")):
    ...
    def submitLayers(self):
        if ((sys.platform.lower() == "win32") or (sys.platform.lower() == "win64")):
            ...
            #print ("Executing: \""+self.RR_ROOT+"/bin/mac64/rrSubmitter.app/Contents/MacOS/rrSubmitter\"  "+self.TempFileName)
            os.system("\""+self.RR_ROOT+"/bin/mac64/rrStartLocal\" rrSubmitter  "+self.TempFileName)
        else:
